...
export const getOwnerDashboardData = async (ownerId: number) => {
  try {
    ...
    const [bookingsResponse, hotelsCountResponse, roomsCountResponse] = await Promise.all([
      supabaseConfig
        .from("bookings")
        .select(`
          *,
...
export const getCustomerDashboardData = async (customerId: number) => {
  try {
      ...
      supabaseConfig
        ...
        .select(`
          *,
          room:rooms(id,name),
          hotel:hotels(id,name),
          owner:user_profiles!bookings_owner_id_fkey(id,name)
        `)
    ...
    const upcomingBookings = bookings.filter(
      (booking) =>
        booking.status !== "cancelled" && booking.start_date > today
    ).length;
